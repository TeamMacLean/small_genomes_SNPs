...
###Calculate ratios and delele those equal to or lower than 1 so only the important contigs remain.
x = 0
dic_ratios = {}
ratios = []
...
ok_hm_p = Stuff.positions_by_fragment(ok_hm, hm_list_2)
# # ok_ht_p = Stuff.positions_by_fragment(ok_ht, ht_list)
...
pp short_sh
dic_short_hm = {}
positions_hm, positions_ht = [], []
short_sh.each do |frag|
	if ok_hm_p.has_key?(frag)
		dic_short_hm.store(frag, ok_hm[frag])
		positions_hm << ok_hm[frag]
	end 
end
